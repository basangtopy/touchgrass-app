...
contract TouchGrass is ReentrancyGuard, Ownable2Step, Pausable {
    ...
    function _getRecoveryDetails(
        bytes32 tokenId
    ) private view returns (TokenRecoveryInfo memory info) {
        TokenConfig memory config = tokenConfigs[tokenId];
        info.symbol = tokenSymbols[tokenId];
        info.tokenAddr = config.tokenAddress;

        if (info.tokenAddr != address(0)) {
            info.balance = IERC20(info.tokenAddr).balanceOf(address(this));
        }

        info.locked = totalLockedByToken[tokenId];
        info.pending = totalPendingWithdrawals[tokenId];
        uint256 protected = info.locked + info.pending;
        info.recoverable = info.balance > protected
            ? info.balance - protected
            : 0;
    }

    /**
     * @dev Helper function to get ETH recovery info for getAllRecoveryStatus
     * @return info ETH recovery information struct
     */
    function _getETHRecoveryInfo()
        private
        view
        returns (ETHRecoveryInfo memory info)
    {
        bytes32 ethId = keccak256(abi.encode("ETH"));
        info.balance = address(this).balance;
        info.locked = totalLockedByToken[ethId];
        info.pending = totalPendingWithdrawals[ethId];
        uint256 ethProtected = info.locked + info.pending;
        if (info.balance > ethProtected) {
            info.recoverable = info.balance - ethProtected;
        }
        // info.recoverable defaults to 0 if not set
    }

    /**
     * @dev Helper function to process a single recovery status entry
     * @param globalIndex The global index being processed
     * @param symbols Array to store symbol
     * @param addresses Array to store address
     * @param contractBalances Array to store balance
     * @param lockedAmounts Array to store locked amount
     * @param pendingAmounts Array to store pending amount
     * @param recoverableAmounts Array to store recoverable amount
     * @param arrayIndex Index in the arrays to write to
     */
    function _processRecoveryStatusEntry(
        uint256 globalIndex,
        string[] memory symbols,
        ...
        if (globalIndex == 0) {
            // ETH
            ETHRecoveryInfo memory ethInfo = _getETHRecoveryInfo();
            symbols[arrayIndex] = "ETH";
            addresses[arrayIndex] = address(0);
            contractBalances[arrayIndex] = ethInfo.balance;
            lockedAmounts[arrayIndex] = ethInfo.locked;
            pendingAmounts[arrayIndex] = ethInfo.pending;
            recoverableAmounts[arrayIndex] = ethInfo.recoverable;
        } else {
            // ERC20
            TokenRecoveryInfo memory info = _getRecoveryDetails(
                supportedTokenIds[globalIndex - 1]
            );
            symbols[arrayIndex] = info.symbol;
            addresses[arrayIndex] = info.tokenAddr;
            contractBalances[arrayIndex] = info.balance;
            lockedAmounts[arrayIndex] = info.locked;
            pendingAmounts[arrayIndex] = info.pending;
            recoverableAmounts[arrayIndex] = info.recoverable;
        }
    }
     ...
     * @return contractBalances Array of contract's balance for each token
     * @return lockedAmounts Array of locked amounts for each token
     * @return pendingAmounts Array of pending withdrawal amounts for each token
     * @return recoverableAmounts Array of recoverable amounts for each token
     */
        ...
        )
    {
        ...
        // Add 1 for ETH
        uint256 length = supportedTokenIds.length + 1;
        totalTokens = length;
        ...
        // Process each entry in separate stack frame
        for (uint256 i = 0; i < resultLength; i++) {
            _processRecoveryStatusEntry(
                _startIndex + i,
                symbols,
                addresses,
                contractBalances,
    ...
    // Helper for hasRecoverableFunds to save stack space
    function _getSingleTokenRecoverableUSD(
        bytes32 tokenId
    ) private view returns (uint256 usdValue) {
        TokenConfig memory config = tokenConfigs[tokenId];
        if (config.tokenAddress == address(0)) return 0; // Skip ETH (handled manually)

        IERC20 token = IERC20(config.tokenAddress);
        uint256 balance = token.balanceOf(address(this));
        uint256 locked = totalLockedByToken[tokenId];
        uint256 pending = totalPendingWithdrawals[tokenId];
        uint256 protected = locked + pending;

        if (balance > protected) {
            uint256 recoverable = balance - protected;
            string memory symbol = tokenSymbols[tokenId];

            try this.getTokenPrice(symbol) returns (uint256 price) {
                // Normalize decimals
                uint256 recoverableIn18;
                if (config.decimals < 18) {
                    ...
                    recoverableIn18 =
                        recoverable /
                        (10 ** (config.decimals - 18));
                }
                usdValue = (recoverableIn18 * price) / 1e18; // PRICE_PRECISION
            } catch {
                usdValue = 0;
            }
        }
    ...
    // Helper to process the recoverable funds loop in a separate stack frame
    function _processRecoverableLoop(
        uint256 start,
        uint256 end
    ) private view returns (bool canRecover, uint256 totalUSD) {
        uint256 len = end - start;
        for (uint256 i = 0; i < len; i++) {
            bytes32 tokenId = supportedTokenIds[start + i];
            uint256 val = _getSingleTokenRecoverableUSD(tokenId);

            if (val > 0) {
                canRecover = true;
                totalUSD += val;
            } else {
                // Fallback check for zero-price tokens
                TokenConfig memory config = tokenConfigs[tokenId];
                if (config.tokenAddress != address(0)) {
                    uint256 bal = IERC20(config.tokenAddress).balanceOf(
                        address(this)
                    );
                    if (
                        bal >
                        totalLockedByToken[tokenId] +
     ...
     * @return canRecover True if any ETH or tokens can be recovered
     * @return totalRecoverableUSD Approximate USD value of recoverable funds
     */
        ...
        )
    {
        ...
        uint256 ethLocked = totalLockedByToken[ethTokenId];
        uint256 ethPending = totalPendingWithdrawals[ethTokenId];
        ...
        if (ethBalance > ethProtected) {
            ...
            // Calculate USD value
            try this.getTokenPrice("ETH") returns (uint256 ethPrice) {
                uint256 recoverableETH = ethBalance - ethProtected;
                totalRecoverableUSD +=
                    (recoverableETH * ethPrice) /
                    PRICE_PRECISION;
            } catch {
                // Skip if price feed fails
            }
        }

        // Check ERC20 tokens (Loop via Helper)
        (bool tokenRecover, uint256 tokenUSD) = _processRecoverableLoop(
            _startIndex,
            endIndex
        );

        canRecover = canRecover || tokenRecover;
        totalRecoverableUSD += tokenUSD;

        return (canRecover, totalRecoverableUSD, totalTokens);
    }
    ...
    // ===== ADDITIONAL FINANCIAL AUDITING HELPER FUNCTIONS =====

    /**
     * @notice Get detailed protection breakdown for a specific token
     * @param _symbol Token symbol
     * @return contractBalance Total balance in contract
     * @return lockedInChallenges Amount locked in active challenges
     * @return pendingAmount Amount in pending withdrawals
     * @return protectedTotal Total protected amount (locked + pending)
     * @return recoverable Amount that can be recovered
     */
        ...
        )
    {
        ...
        lockedInChallenges = totalLockedByToken[tokenId];
        pendingAmount = totalPendingWithdrawals[tokenId];
    ...
    /**
     * @dev Helper function to calculate ETH protection values for global summary
     * @return result Bundled protection calculation result for ETH
     */
    function _calculateETHProtection()
        private
        view
        returns (ProtectionCalcResult memory result)
    {
        bytes32 ethTokenId = keccak256(abi.encode("ETH"));
        uint256 ethBalance = address(this).balance;
        uint256 ethLocked = totalLockedByToken[ethTokenId];
        ...
        try this.getTokenPrice("ETH") returns (uint256 ethPrice) {
            result.contractValueUSD = (ethBalance * ethPrice) / PRICE_PRECISION;
            result.lockedUSD = (ethLocked * ethPrice) / PRICE_PRECISION;
            result.pendingUSD = (ethPending * ethPrice) / PRICE_PRECISION;
        } catch {
            // Skip if price feed fails - values remain 0
        }
        return result;
    }

    /**
     * @dev Helper function to calculate single ERC20 token protection values
     * @param tokenId Token ID to calculate for
     * @return result Bundled protection calculation result
     */
    function _calculateSingleTokenProtection(
        bytes32 tokenId
    ) private view returns (ProtectionCalcResult memory result) {
        TokenConfig memory config = tokenConfigs[tokenId];

        if (config.tokenAddress == address(0)) return result; // Skip ETH
        ...
        try this.getTokenPrice(symbol) returns (uint256 price) {
            ...
            if (config.decimals < 18) {
                uint256 multiplier = 10 ** (18 - config.decimals);
                balanceIn18 = balance * multiplier;
                lockedIn18 = locked * multiplier;
                pendingIn18 = pending * multiplier;
            } else {
                uint256 divisor = 10 ** (config.decimals - 18);
                balanceIn18 = balance / divisor;
                lockedIn18 = locked / divisor;
                pendingIn18 = pending / divisor;
            }

            result.contractValueUSD = (balanceIn18 * price) / PRICE_PRECISION;
            result.lockedUSD = (lockedIn18 * price) / PRICE_PRECISION;
            result.pendingUSD = (pendingIn18 * price) / PRICE_PRECISION;
        } catch {
            // Skip if price feed fails
        }
        return result;
    ...
    /**
     * @dev Helper function to calculate all ERC20 tokens protection values
     * @return result Bundled protection calculation result for all ERC20 tokens
     */
    function _calculateAllERC20Protection()
        private
        view
        returns (ProtectionCalcResult memory result)
    {
        for (uint256 i = 0; i < supportedTokenIds.length; i++) {
            ProtectionCalcResult
                memory tokenResult = _calculateSingleTokenProtection(
                    supportedTokenIds[i]
                );
            result.contractValueUSD += tokenResult.contractValueUSD;
            result.lockedUSD += tokenResult.lockedUSD;
            result.pendingUSD += tokenResult.pendingUSD;
        }
        return result;
    }
     ...
     * @return totalContractValueUSD Total USD value in contract
     * @return totalLockedUSD Total USD value locked in challenges
     * @return totalPendingUSD Total USD value in pending withdrawals
     * @return totalProtectedUSD Total USD value protected (locked + pending)
     * @return totalRecoverableUSD Total USD value recoverable
     */
    ...
    {
        // Calculate ETH protection in separate stack frame
        ProtectionCalcResult memory ethResult = _calculateETHProtection();
        totalContractValueUSD = ethResult.contractValueUSD;
        totalLockedUSD = ethResult.lockedUSD;
        totalPendingUSD = ethResult.pendingUSD;

        // Calculate all ERC20 tokens in separate stack frame
        ProtectionCalcResult
            memory erc20Result = _calculateAllERC20Protection();
        totalContractValueUSD += erc20Result.contractValueUSD;
        totalLockedUSD += erc20Result.lockedUSD;
        totalPendingUSD += erc20Result.pendingUSD;

        totalProtectedUSD = totalLockedUSD + totalPendingUSD;

        if (totalContractValueUSD > totalProtectedUSD) {
            totalRecoverableUSD = totalContractValueUSD - totalProtectedUSD;
        }
